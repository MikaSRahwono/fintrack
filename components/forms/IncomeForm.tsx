...
import { INCOME_CATEGORIES, ACCOUNTS } from "@/lib/constants";
...
interface Props {
  open: boolean;
  onClose: () => void;
  onSave: (form: IncomeForm) => Promise<void>;
  initial?: Income | null;
}

const empty = (): IncomeForm => ({
  date: todayStr(), name: "", income_category: INCOME_CATEGORIES[0],
  account: ACCOUNTS[0], amount: 0,
});

export default function IncomeFormModal({ open, onClose, onSave, initial }: Props) {
  const [form, setForm] = useState<IncomeForm>(initial ? {
    date: initial.date, name: initial.name, income_category: initial.income_category,
    account: initial.account, amount: initial.amount,
  } : empty());
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const set = (k: keyof IncomeForm) => (v: string | number) =>
    setForm(f => ({ ...f, [k]: v }));
  ...
  return (
    <Modal open={open} onClose={onClose} title={initial ? "Edit Income" : "Add Income"} icon="↑">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="col-span-2">
          <Input label="Name / Description" value={form.name}
            onChange={e => set("name")(e.target.value)} placeholder="e.g. Gaji bulan ini" />
        </div>
        <Select label="Income Category" options={INCOME_CATEGORIES} value={form.income_category}
